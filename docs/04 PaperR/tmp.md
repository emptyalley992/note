...
你是一个光电方向的专家，同时也是一个ai专家，研究方向为二者的交叉，你当前在进行一个实验，对番茄喷洒农药，然后对齐进行光谱分析，根据光谱结果来识别所喷洒的农药的种类和浓度。从ai的角度理解，我现在有一堆样本数据，其存储在excel表格中，其中每行是一个样本（首行是列名），最后一列是分类的label，例如，0表示未喷洒农药，ben-0.5表示喷洒ben种类农药其浓度为0.5；bi-1表示喷洒bi种类农药，浓度为1。前面438列表示不同的频段。

你现在面临的任务是对“笨+吡原始数据.xlsx”里的原始数据进行合适的预处理（可能包括对光谱数据的特殊处理，如多元散射校正MSC等），然后通过机器学习或深度学习方法实现分类的需求。我需要你先为我实现SVM方法的python代码，并选择对应的需要的数据预处理方法，可能包括对光谱数据的特殊处理（不属于常见机器学习范畴的）。

# 数据说明
文件示例“笨+吡原始数据.xlsx”，该文件只是少量的样本例子，实际数据有 1800个。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjIzNTI4NDI0LDIwMjM4Njk5MjgsLTE0Nz
IzMDAwMjEsLTg1MjEyMjM4LC0xMjc2NDUwMjk2LC02MTMzNjc3
MDksMTc3ODEyOTQ2LC04NDI3MDE2NTJdfQ==
-->